...
)
{
	...
	// URP reflection probe sampling using GlossyEnvironmentReflection from Lighting.hlsl
	half3 reflectVector = refl;
	half perceptualRoughness = _Roughness;
	half3 irradiance = GlossyEnvironmentReflection(reflectVector, perceptualRoughness, 1.0);
	skyColour = irradiance;
#else
	...
	skyColour += pow(max(0., dot(refl, i_lightDir)), fallOff) * _DirectionalLightBoost * _LightColor0.rgb * i_shadow;
#endif
